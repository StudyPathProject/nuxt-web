<template>
  <Disclosure as="nav" class="bg-[#26B89B]" v-slot="{ open }">
      ...
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
          <div class="flex flex-shrink-0 items-center">
            ...
            <NuxtLink to="/" class="pl-2 capitalize">
              <span class="text-white pr-1 font-bold">STUDY</span>
              <span class="text-[#3D5B89] font-bold">PATH</span>
            </NuxtLink>
          </div>
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <NuxtLink v-for="item in navigation" :key="item.name" :to="item.href" class="transition-all"
                :class="[item.current ? 'bg-[#26B89B] text-white' : 'text-[#3D5B89] hover:bg-[#2e8a7c67] hover:text-[#2d4261]', 'rounded-md px-3 py-2 text-sm font-medium']"
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          <UButton class="text-white py-2 px-4 bg-[#3D5B89]" variant="main">Iniciar Sesión</UButton>
